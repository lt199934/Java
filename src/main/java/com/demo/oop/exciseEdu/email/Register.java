...
public class Register {
		public boolean vreify(User user){
			...
			int email1=user.email.indexOf("@");
			if(email1==-1){
				System.out.println("必须包含@");
				return false;
			}
			int email2=user.email.indexOf(".");
			if(email2==-1){
				System.out.println("必须包含.");
				return false;
			}
			...
			if(email1>=email2){
				System.out.println(".必须在@的后面");
				return false;
			}
			String emaila=user.email.substring(0,email1);
			if("".equals(emaila)){
				System.out.println("@前面必须要有内容");
				return false;
			}
			String emailb=user.email.substring(email1+1,email2);
			if("".equals(emailb)){
				System.out.println("@和.中间必须要有内容");
				return false;
			}
			String emailc=user.email.substring(email2+1);
			if(!"com".equals(emailc)&&!"cn".equals(emailc)){
				System.out.println(".后面必须为com或cn");
				return false;
			}
			return true;
			}
